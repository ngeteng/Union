...
async function main() {
  ...
  // Inisialisasi wallets sama pilihan menu
  const wallets = [];
  let idx = 1;
  while (true) {
  ...
  while (batchCount < maxBatches) {
    logger.info(`=== Batch ke-${batchCount + 1}/${maxBatches} ===`);

    const choice = 3; // atau baca input user
    ...
    for (const walletInfo of wallets) {
      if (
        ...
        logger.warn(`Skipping wallet '${walletInfo.name}': Invalid private key.`);
        continue;
      }

      if (choice === 1) {
        await sendFromWallet(walletInfo, 1, 'holesky');
      } else if (choice === 2) {
        await sendFromWallet(walletInfo, 1, 'babylon');
      } else {
        const destinations = ['holesky', 'babylon']
          .filter(d => d !== 'babylon' || walletInfo.babylonAddress);
