...
async function sendFromWallet(walletInfo, maxTransaction, destination) {
  ...
  // --- Penentuan Tujuan (Babylon/Holesky) ---
  ...
  for (let i = 1; i <= maxTransaction; i++) {
    ...
    try {
      ...
      // --- Parse Event untuk Ambil Jumlah ---
      ...
      // ===          MEMULAI POLLING             ===
      // ============================================
      logger.loading(`  ⮡ Polling for Union packet hash... (max 50 tries, ~4 mins)`);
      ...
      if (packetHash) {
        ...
        bufferReport(`✅ Sent *${amount} USDC* → *${destinationName}* | Packet: \`${packetHash}\``);
      } else {
        logger.warn(`  ⮡ Could not retrieve Union packet hash.`);
        bufferReport(`✅ Sent *${amount} USDC* → *${destinationName}* | ⚠ Packet hash not found.`);
      }
      // ============================================
      // ===          SELESAI POLLING             ===
      // ============================================
      ...
      bufferReport(`❌ Failed send from *${walletInfo.name || 'Unnamed'}* → *${destinationName}*: ${err.message.substring(0, 100)}...`); // Batasi panjang pesan error jika perlu
      console.log('');
    }

    // --- Flush Laporan ke Telegram ---
    await flushReport();
